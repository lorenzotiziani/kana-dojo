...
const OVERVIEW_CARD_HALO_GAP = 8;
...
export default function OverviewStatsCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'rounded-(--overview-card-outer-radius) border-4 border-(--border-color) p-(--overview-card-halo-gap)',
        className,
      )}
      style={
        {
          '--overview-card-halo-gap': `${OVERVIEW_CARD_HALO_GAP}px`,
          '--overview-card-outer-radius':
            'calc(var(--radius-3xl) + var(--overview-card-halo-gap))',
          '--overview-card-inner-radius':
            'calc(var(--overview-card-outer-radius) - var(--overview-card-halo-gap))',
        } as React.CSSProperties
      }
    >
      <div
        className={cn(
          'group relative overflow-hidden rounded-(--overview-card-inner-radius)',
